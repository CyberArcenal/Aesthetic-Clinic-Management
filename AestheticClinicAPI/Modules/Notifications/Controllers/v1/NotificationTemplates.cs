...
namespace AestheticClinicAPI.Modules.Notifications.Controllers.v1
{
    ...
    public class NotificationTemplatesController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<
            ActionResult<ApiResponse<PaginatedResult<NotificationTemplateResponseDto>>>
        > GetAll(
        ...
        )
        {
            ...
            if (!result.IsSuccess)
                return BadRequest(
                    ApiResponse<PaginatedResult<NotificationTemplateResponseDto>>.Fail(
                        result.ErrorMessage!
                    )
                );
            return Ok(
                ApiResponse<PaginatedResult<NotificationTemplateResponseDto>>.Ok(result.Data!)
            );
        }

        [HttpGet("{id}")]
        public async Task<ActionResult<ApiResponse<NotificationTemplateResponseDto>>> GetById(
            int id
        )
        {
            ...
            if (!result.IsSuccess)
                return NotFound(
                    ApiResponse<NotificationTemplateResponseDto>.Fail(result.ErrorMessage!)
                );
            return Ok(ApiResponse<NotificationTemplateResponseDto>.Ok(result.Data!));
        }

        [HttpPost]
        public async Task<ActionResult<ApiResponse<NotificationTemplateResponseDto>>> Create(
        ...
        )
        {
            ...
            return Ok(
                ApiResponse<NotificationTemplateResponseDto>.Ok(result.Data!, "Template created.")
            );
        }

        [HttpPut("{id}")]
        public async Task<ActionResult<ApiResponse<NotificationTemplateResponseDto>>> Update(
            int id,
        ...
        )
        {
            ...
            return Ok(
                ApiResponse<NotificationTemplateResponseDto>.Ok(result.Data!, "Template updated.")
            );
        }
