...
namespace AestheticClinicAPI.Modules.Notifications.Controllers.v1
{
    ...
    public class NotifyLogsController : ControllerBase
    {
        ...
        {
            var result = await _notifyLogService.GetPaginatedAsync(
                page,
                pageSize,
                recipientEmail,
                status,
            ...
            if (!result.IsSuccess)
                return BadRequest(
                    ApiResponse<PaginatedResult<NotifyLogResponseDto>>.Fail(result.ErrorMessage!)
                );
        ...
        public async Task<ActionResult<ApiResponse<NotifyLogResponseDto>>> GetById(int id)
        {
            ...
            if (!result.IsSuccess)
                return NotFound(ApiResponse<NotifyLogResponseDto>.Fail(result.ErrorMessage!));
